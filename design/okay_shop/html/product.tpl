    ...
    <div class="fn_product product" itemscope itemtype="http://schema.org/Product">
        ...
        <h1 class="product_heading">
            <span data-product="{$product->id}"
                  itemprop="name">{$product->name|escape} {if $product->variants|count == 1 && !empty($product->variant->name)}({$product->variant->name|escape}){/if}</span>
        </h1>
        ...
        <div class="sku clearfix">
            {if $brand}
                <div class="product_brand_block clearfix">
                    <span>{$lang->product_brand_name} <a
        ...
        <div class="row fn_transfer">
            <div class="col-lg-5 col-xl-6">
                <div class="product_image">
                    {* Main product image *}
                    {if $product->image}
                        <a href="{$product->image->filename|resize:800:600:w}" data-fancybox="group"
                    ...
                    {else}
                        <img class="fn_img" src="design/{$settings->theme}/images/no_image.png" width="340" height="340"
                    ...
                    <div class="fn_images images clearfix">
                        ...
                        {foreach $product->images|cut as $i=>$image}
                            <div class="images_item">
                                <a class="images_link" href="{$image->filename|resize:800:600:w}" data-fancybox="group"
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <div>
                                ...
                                {else}
                                    <a href="#" data-id="{$product->id}" class="fn_wishlist product_wish"
                        ...
                        <div class="col-sm-6">
                            <div class="clearfix">
                                <span class="details_label" data-language="product_rating">{$lang->product_rating}
                                    :</span>
                                ...
                                <div id="product_{$product->id}"
                                    ...
                                    {if $product->rating > 0}
                                        <span class="hidden"
                                              itemprop="reviewCount">{$product->votes|string_format:"%.0f"}</span>
                                        <span class="hidden"
                                              itemprop="ratingValue">({$product->rating|string_format:"%.1f"})</span>
                    ...
                    <form class="fn_variants" action="/{$lang_link}cart">
                                    ...
                                    <select name="variant"
                                        ...
                                        {foreach $product->variants as $v}
                                            <option value="{$v->id}" data-price="{$v->price|convert}"
                                                    data-stock="{$v->stock}"{if $v->compare_price > 0} data-cprice="{$v->compare_price|convert}"{/if}{if $v->sku} data-sku="{$v->sku|escape}"{/if} {if $v->units}data-units="{$v->units}"{/if}>{if $v->name}{$v->name|escape}{else}{$product->name|escape}{/if}</option>
                                        {/foreach}
                                    </select>
                                {/if}
                            </div>
                            ...
                            <div class="col-sm-6 fn_is_stock{if $product->variant->stock < 1} hidden{/if}">
                                <span class="details_label quontity_label" data-language="product_quantity">
                                    {$lang->product_quantity}<span
                                            class="fn_units">{if $product->variant->units}, {$product->variant->units|escape}{/if}</span>:
                                </span>
                        ...
                        <div class="row" itemprop="offers" itemscope="" itemtype="http://schema.org/Offer">
                            ...
                            <div class="col-sm-6 product_buttons">
                                ...
                                {* Submit button *}
                                <button class="fn_is_stock button product_btn{if $product->variant->stock < 1} hidden{/if}"
                                        type="submit"
                                    ...
                                    {foreach $comments as $comment}
                                        ...
                                        <div class="comment_item{if $level > 0} admin_note{/if}">
                                            ...
                                            <div class="comment_footer">
                                                {* Comment date *}
                                                <span class="comment_date">{$comment->date|date}
                                                    , {$comment->date|time}</span>
                                                {* Comment status *}
                                                {if !$comment->approved}
                                                    <span data-language="post_comment_status">({$lang->post_comment_status}
                                                        )</span>
                                                {/if}
                                            </div>
            ...
            <nav>
                <ol class="pager row">
                    <li class="col-xs-12{if $next_product} col-sm-6{else} col-sm-12{/if}">
                        {if $prev_product}
                            <a href="{$lang_link}products/{$prev_product->url}">←
                                <span>{$prev_product->name|escape}</span></a>
                        {/if}
                    </li>
                    <li class="col-xs-12 col-sm-6">
                        {if $next_product}
                            <a href="{$lang_link}products/{$next_product->url}"><span>{$next_product->name|escape}</span>
                                →</a>
                        {/if}
                    </li>
                </ol>
