    ...
    <div class="col-lg-7 col-md-7">
        <div class="heading_page">
            ...
            <div class="export_block export_users hint-bottom-middle-t-info-s-small-mobile  hint-anim"
                 data-hint="{$btr->general_example|escape}">
                ...
                <div class="col-lg-12 col-md-12">
                    <div class="boxed fn_toggle_wrap ">
                        <div class="heading_box boxes_inline">
                            {$btr->import_file|escape} {$file->name|escape}
                            ({($file->size/1024)|round:'2'} {$btr->general_kb|escape})
                        </div>
                        {if $filename}
                            ...
                            <div class="okay_list">
                                <div class="okay_list_head">
                                    ...
                                    <div class="col-lg-6 col-md-6">
                                        ...
                                        <div>
                                            <a href="javascript:;"
                                               class="btn btn_small btn-info fn_skip_all">{$btr->import_skip_all}</a>
                                            <a href="javascript:;"
                                               class="btn btn_small btn-info fn_new_all">{$btr->import_new_all}</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="">
                                    {foreach $source_columns as $column}
                                        <div class="row import_row {if $column@first}mt-1{/if}">
                                            ...
                                            <div class="col-lg-6 col-md-6 col-sm-12 text-xs-left fn_row">
                                                <input type="hidden" name="csv_fields[{$column->name|escape}]"
                                                       value="{$column->value|escape}"/>
                                                <a href="javascript:;"
                                                   class="fn_edit_column {if empty($column->value)}text_warning{elseif $column->is_nf_selected}text_green{/if}"
                                    ...
                                    <select class="selectpicker fn_select" data-live-search="true">
                                        <optgroup label="{$btr->import_additional}">
                                            <option value=""
                                                    data-label="{$btr->import_skip}">{$btr->import_skip}</option>
                                            <option value="" data-label="{$btr->import_new_feature}"
                                        ...
                                        <optgroup label="{$btr->import_main_fields}">
                                            {foreach $columns_names as $cname}
                                                <option value="{$cname}"
                                                        data-label="{$btr->get_translation('import_field_'|cat:$cname)}">
                                    ...
                                    <div class="col-lg-12 col-md-12 mt-1">
                                        <button type="submit" name="import" value="1"
                                                class="btn btn_small btn_blue float-md-right"><i
                                                    class="fa fa-upload"></i>&nbsp;{$btr->import_do_import}</button>
                                    </div>
                                </div>
                            </div>
                        {elseif $import}
                            <div id='import_result' class="boxes_inline" style="display: none;">
                                <a class="btn btn_small btn-info" href="index.php?module=ImportLogAdmin"
                                   target="_blank">{$btr->import_log|escape}</a>
                            </div>
                            <div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-7 col-md-6 my-h">
                                <button type="submit" class="btn btn_small btn_blue float-md-right"><i
                                            class="fa fa-upload"></i>&nbsp;{$btr->import_to_download|escape}</button>
                            </div>
                        </div>
                    </div>
                </div>
